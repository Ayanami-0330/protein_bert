...
SEED = 22
MODEL_KEY = "ProtT5_freezed"
MODEL_NAME = "Rostlab/prot_t5_xl_uniref50"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
DTYPE = torch.float16 if DEVICE == "cuda" else torch.float32
_PROTT5_CACHE: Dict[str, Any] = {"tokenizer": None, "model": None}
...
def load_prott5_components():
    if _PROTT5_CACHE["model"] is not None:
        return _PROTT5_CACHE["tokenizer"], _PROTT5_CACHE["model"]

    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
    model = T5EncoderModel.from_pretrained(MODEL_NAME, torch_dtype=DTYPE, use_safetensors=False)
    model = model.to(DEVICE)
    model.eval()
    _PROTT5_CACHE["tokenizer"] = tokenizer
    _PROTT5_CACHE["model"] = model
    return tokenizer, model


def encode_prott5_sentence(seqs, batch_size: int = 2, max_length: int = 1024) -> np.ndarray:
    tokenizer, model = load_prott5_components()
...
def main() -> None:
    ...
    for split_name, seqs in split_seqs.items():
        ...
        raw[split_name] = load_or_compute(raw_path, lambda seqs=seqs: encode_prott5_sentence(seqs))
